...
import { productCategoryService } from "@/lib/api-client"
...
import { ProductCategory as ApiProductCategory, ApiError } from "@/types/api"
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs"
import { Textarea } from "@/components/ui/textarea"

interface ProductCategory extends ApiProductCategory {
  products: string[]
}
...
export default function ProductCategoriesPage() {
  ...
  const fetchCategories = async () => {
    try {
      setLoading(true)
      const response = await productCategoryService.findAll()
      setCategories(response as ProductCategory[])
    } catch (error: unknown) {
      const apiError = error as ApiError
      toast({
        title: "Error",
        description: apiError.response?.data?.message || "Failed to fetch categories",
        variant: "destructive",
      })
    } finally {
      setLoading(false)
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: apiError.response?.data?.message || "Failed to create category",
        variant: "destructive",
      })
    }
  }
  ...
  const handleEdit = async () => {
    ...
    try {
      await productCategoryService.update(
        selectedCategory.productCategoryId ?? selectedCategory._id,
        payload,
      )
      toast({
        title: "Success",
      ...
      toast({
        title: "Error",
        description: apiError.response?.data?.message || "Failed to update category",
        variant: "destructive",
      })
    }
  }
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (item: Record<string, unknown>) => {
        const category = item as unknown as ProductCategory
        return category.name_ar || <span className="text-muted-foreground">Not provided</span>
      },
    },
    {
      key: "image",
      ...
      render: (item: Record<string, unknown>) => {
        ...
        return category.image ? (
          <Image src={category.image} alt={category.name_en || category.name} width={40} height={40} className="w-10 h-10 object-cover rounded" />
        ) : (
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div>
              <ImageUpload
                ...
                uploadEndpoint={`${process.env.NEXT_PUBLIC_API_URL}/v1/product-category/upload-image`}
      ...
      <ConfirmDialog
        ...
        description={`Are you sure you want to delete "${selectedCategory?.name_en || selectedCategory?.name}"? This action cannot be undone.`}
        confirmText="Delete"
        cancelText="Cancel"
